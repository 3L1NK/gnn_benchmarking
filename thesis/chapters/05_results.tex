This chapter reports benchmark outcomes generated from \texttt{results/results\_tuned\_all.jsonl} through the hardened reporting pipeline. All tables and figures are synchronized from \texttt{results/reports/thesis\_tuned\_all/}. Every summary in this chapter is derived from the same canonical deduplicated dataframe (\texttt{master\_comparison.csv}) and checked against the 26-row run matrix.
...
Across the 13 model variants, moving from daily to weekly rebalancing improves annualized Sharpe and reduces turnover for the majority of runs. This is consistent with lower implementation friction from less frequent trading.
