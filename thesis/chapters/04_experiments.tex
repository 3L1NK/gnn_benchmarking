...
The tuned benchmark matrix contains 13 model variants and two rebalance policies (1 and 5), yielding 26 canonical rows after deduplication by stable run key.

Model variants:
\begin{itemize}
  \item \textbf{Non-graph}: \texttt{xgb\_raw}, \texttt{lstm}
  \item \textbf{Graph-feature}: \texttt{xgb\_node2vec\_corr}
...
\begin{itemize}
  ...
  \item expected run-matrix size checks (26 rows for tuned-all).
\end{itemize}
